          ...
          messages: [
            {
              ...
              content: [
                ...
                {
                  type: 'text',
                  text: `Analyze this receipt image and extract ALL the information including any refunds, returns, or credits. Return ONLY a valid JSON object with no other text or formatting. DO NOT use markdown code blocks or backticks.

IMPORTANT: 
- For regular purchases, the price should be POSITIVE
- For refunds, returns, or credits, the price should be NEGATIVE
- Look for words like "REFUND", "RETURN", "CREDIT" or negative signs on the receipt
...
Your response must be ONLY this JSON structure:
{
  ...
  "date": "date or null",
  "items": [
    {"name": "item name", "price": number (positive for purchases, negative for refunds)}
  ],
...
CRITICAL: Your entire response must be valid JSON only. Do not include any text before or after the JSON object.`
  ...
  const calculateSplit = () => {
    ...
    // Handle tax and fees
    const assignedItemsSubtotal = assignedItemsTotal;
    const taxAndFees = receiptData.total - (receiptData.subtotal || assignedItemsTotal);
    
    // Split tax and fees equally
    if (taxAndFees > 0) {
      const perPersonTax = taxAndFees / validPeople.length;
      validPeople.forEach(person => {
        personTotals[person] += perPersonTax;
      });
    }
                  ...
                  <div className="bg-gray-50 rounded-xl p-6">
                    ...
                    <div className="space-y-3">
                      {receiptData.items.map((item, itemIndex) => (
                        <div key={itemIndex} className={`bg-white p-4 rounded-lg border-2 ${
                          item.price < 0 ? 'border-green-500 bg-green-50' : 'border-gray-200'
                        }`}>
                          <div className="flex justify-between items-start mb-2">
                            <span className="font-medium text-gray-800">
                              {item.name}
                              {item.price < 0 && <span className="ml-2 text-xs bg-green-600 text-white px-2 py-1 rounded">REFUND</span>}
                            </span>
                            <span className={`font-bold ${item.price < 0 ? 'text-green-600' : 'text-indigo-600'}`}>
                ...
                <div className="space-y-6">
                  ...
                  {split && (
                    <div className="bg-indigo-50 rounded-xl p-6">
                      ...
                      {split.hasUnassignedItems && (
                        <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-4 mb-4">
                          <p className="text-yellow-800 font-semibold mb-2">⚠️ Warning: Unassigned Items</p>
                          <p className="text-sm text-yellow-700 mb-2">
                            The following items have not been assigned to anyone:
                          </p>
                          <ul className="text-sm text-yellow-700 space-y-1">
                            {split.unassignedItems.map((item, index) => (
                              <li key={index}>• {item.name} (${item.price.toFixed(2)})</li>
                            ))}
                          </ul>
                          <p className="text-sm text-yellow-700 mt-2 font-medium">
                            Please assign these items to people, or they will not be included in the bill.
                          </p>
                        </div>
                      )}
                      ...
                      <div className="space-y-3">
                        {split.people.map((person, index) => {
                          const amount = split.personTotals[person];
                          const isNegative = amount < 0;
                          return (
                            <div key={index} className="bg-white rounded-lg p-4 shadow-sm">
                              <div className="flex justify-between items-center">
                                <span className="font-semibold text-gray-800">{person}</span>
                                <span className={`text-2xl font-bold ${isNegative ? 'text-green-600' : 'text-red-600'}`}>
                                  ${amount.toFixed(2)}
                                </span>
                              </div>
                            </div>
                      ...
                      <div className="mt-4 pt-4 border-t-2 border-indigo-200">
                        <div className="flex justify-between items-center">
                          <span className="font-bold text-gray-800">Total Assigned:</span>
                          <span className="text-2xl font-bold text-gray-800">
                            ${Object.values(split.personTotals).reduce((sum, val) => sum + val, 0).toFixed(2)}
                          </span>
                        </div>
                        {split.hasUnassignedItems && (
                          <p className="text-sm text-yellow-600 mt-2">
                            Note: ${(split.total - Object.values(split.personTotals).reduce((sum, val) => sum + val, 0)).toFixed(2)} from unassigned items not included
                          </p>
                        )}
                      </div>
                    </div>
